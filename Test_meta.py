...
if __name__ == '__main__':
    ...
    parser.add_argument('--K_hots', type=int, default=10, help='number of the K hots')
    ...
    parser.add_argument('--model_dir', type=str, default='./data/model_ped2_900_exp_k1' ,help='directory of model')
    ...
    videos = OrderedDict()
    video_change = glob.glob(os.path.join(test_folder, '*'))
    for index, _ in enumerate(video_change):
        video_change[index] = video_change[index].replace('\\', '/')
    videos_list = sorted(video_change)
    ...
    for video in videos_list:
        ...
        video_tran = glob.glob(os.path.join(video, '*.tif'))
        for index, _ in enumerate(video_tran):
            video_tran[index] = video_tran[index].replace('\\', '/')
        videos[video_name]['frame'] = video_tran
