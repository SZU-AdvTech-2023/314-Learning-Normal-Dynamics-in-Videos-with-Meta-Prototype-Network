...
parser.add_argument('--model_dir', type=str, default='./data/model_900_exp', help='directory of model')
...
if __name__ == '__main__':
    ...
    if 'shanghai' in args.dataset_type or 'ped1' in args.dataset_type:
        labels = np.expand_dims(labels, 0)

    videos = OrderedDict()
    video_tran = glob.glob(os.path.join(test_folder, '*'))
    for index, _ in enumerate(video_tran):
        video_tran[index] = video_tran[index].replace('\\', '/')
    videos_list = sorted(video_tran)
    ...
    for video in videos_list:
        ...
        video_frame = glob.glob(os.path.join(video, '*.jpg'))
        for index, _ in enumerate(video_tran):
            video_frame[index] = video_frame[index].replace('\\', '/')
        videos[video_name]['frame'] = video_frame
        for num in range(len(videos[video_name]['frame'])):
            videos[video_name]['frame'][num] = videos[video_name]['frame'][num]
    ...
    ckpt = snapshot_path + '/model_900.pth'
    ckpt_name = ckpt.split('/')[-1].split('_')[-1]
    print("ckpt:" + ckpt)
